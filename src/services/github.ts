...
export async function fetchRepoData(owner: string, repo: string, env: Env, etag: string | null = null) {
    ...
    //if etag is available in cache, add it to the request headers
    ...
    //check if data was modified since last fetch
    if (res.status == 304) {
    ...
    const data = {
        ...
        isOwnedByOrg: rawData.owner.type === 'Organization',
...
export async function fetchUserData(username: string, env: Env) {
    ...
    `
    //graphql query to fetch user contributions data
    ...
    try {
        const userResult = await octokit.graphql.paginate(USER_QUERY, {login: username}) as any;
        ...
        const contribResult = await octokit.graphql(CONTRIBUTION_QUERY, {login: username}) as any;
        ...
        const isNotFound = e?.errors?.some((err: any) => err.type === 'NOT_FOUND');
        if (isNotFound) {
            return {userExists: false as const}
        } else {
            throw e
